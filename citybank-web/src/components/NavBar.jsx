...
const NavBar = () => {
  ...
  return (
    <div className='w-full flex flex-row'>
        <nav className='flex-1 py-6 justify-between items-center navbar'>
            ...
            <ul className='list-none sm:flex hidden justify-left items-center flex-1'>
                {navLinks.map((nav, index) => (
                    <li
                        key={nav.id}
                        className={`cursor-pointer text-[20px] ${index===navLinks.length-1?'mr-0':'mr-10'} text-slate-100 font-semibold`}
                    >
                        <a href={`#${nav.id}`}>
                            {nav.title}
                        </a>
        ...
        <div className='flex text-slate-100 items-center text-[20px]'>How can we help?  <MagnifyingGlassIcon className="h-6 w-6 ml-1 text-bold" /></div>
    </div>
  )
}
